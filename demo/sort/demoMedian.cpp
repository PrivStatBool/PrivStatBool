...
int main() {
    ...
    ctx_answer = HomMedian(ciphertext, length, &key->cloud);
    ...
    std::vector<int> medianResultPlain = decryptToBinaryVector(ctx_answer, length, key);
    double medianResult = decodeDouble(medianResultPlain);    

    std::cout << "Decrypted Result: " << medianResult << std::endl;

    // Calculate and display the plain skewness for comparison
    double answerPlain = plainMedian(dataset);
    std::cout << "Exact Results: " << answerPlain << std::endl;

    // Accuracy using MAPE
    double accuracy = calculateMAPEScalr(answerPlain, medianResult); 
