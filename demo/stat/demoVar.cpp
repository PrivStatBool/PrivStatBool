...
int main() {
    std::cout << "Demo: Variance of a Dataset" << std::endl;
    ...
    // Demo: Find Mean
    ctxAnswer = HomVar(encData, length, &key->cloud); 
    ...
    std::vector<double> plainVars = plainVar(dataset); 
    printVector(plainVars, "Exact Result:");

    // Accuracy using MAPE
    double accuracy = evaluateAccuracy(decryptedResults, plainVars); 
