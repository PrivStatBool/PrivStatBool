...
int main() {
    std::cout << "Demo: Sum of elements" << std::endl;
    ...
    // Demo: Find Mean
    ctx_answer = HomSum(EncData, length, &key->cloud); 
    ...
    std::vector<double> plainSums = plainSum(dataset); // Implement this function similar to plainSum
    printVector(plainSums, "Exact Result:");

    // Accuracy using MAPE
    double accuracy = evaluateAccuracy(decryptedResults, plainSums); // Ensure this function calculates MAPE
